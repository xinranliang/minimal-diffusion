...
#SBATCH --output=/n/fs/xl-diffbia/projects/minimal-diffusion/slurm_output/2023-01-22/cifar10_cond_gray.txt            # where stdout and stderr will write to
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m torch.distributed.launch --nproc_per_node=8 --master_port 8103 main.py \
    ...
    --ckpt-name cifar10_cond_color0.01_gray0.99_epoch1000_ema \
    --pretrained-ckpt ./logs/2023-01-22/cifar10/color0.01_gray0.99/UNet_diffusionstep_1000_samplestep_1000_condition_True_lr_0.0001_bs_256/ckpt/epoch_1000_ema_0.9995.pth

CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m torch.distributed.launch --nproc_per_node=8 --master_port 8103 main.py \
    --arch UNet --dataset cifar10 --batch-size 4096 --num-sampled-images 50000 --sampling-gray-only --sampling-steps 250 --class-cond \
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m torch.distributed.launch --nproc_per_node=8 --master_port 8103 main.py \
    ...
    --ckpt-name cifar10_cond_color0.99_gray0.01_epoch1000_ema \
    --pretrained-ckpt ./logs/2023-01-22/cifar10/color0.01_gray0.99/UNet_diffusionstep_1000_samplestep_1000_condition_True_lr_0.0001_bs_256/ckpt/epoch_1000_ema_0.9995.pth
