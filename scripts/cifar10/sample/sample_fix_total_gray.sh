...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m torch.distributed.launch --nproc_per_node=8 --master_port 8103 main.py \
    --arch UNet --dataset cifar10 --batch-size 10000 --num-sampled-images 50000 --sampling-gray-only --sampling-steps 250 \
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m torch.distributed.launch --nproc_per_node=8 --master_port 8103 main.py \
    ...
    --pretrained-ckpt ./logs/2023-03-27/cifar10/color0.7_gray0.3/UNet_diffusionstep_1000_samplestep_1000_condition_True_lr_0.0001_bs_256_dropprob_0.1/ckpt/epoch_1000_ema_0.9995.pth
