...
#SBATCH --output=/n/fs/xl-diffbia/projects/minimal-diffusion/slurm_output/2023-01-05/cifar10_train_color0.99_gray0.01_sample_gray.txt            # where stdout and stderr will write to
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 --master_port 8103 main.py \
    --arch UNet --dataset cifar10 --batch-size 2048 --num-sampled-images 10000 --sampling-gray-only --sampling-steps 250 \
