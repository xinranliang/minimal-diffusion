...
#SBATCH --output=/n/fs/xl-diffbia/projects/minimal-diffusion/slurm_output/2023-07-31/train_checkdomainclassifier_cifar0.9_imagenet0.1_cond_droprob0.1.txt            # where stdout and stderr will write to
...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 --master_port $3 main.py \
    --arch UNet --dataset cifar-imagenet-check --epochs 1000 --batch-size 256 --lr 1e-4 --sampling-steps 250 \
    --data-dir ./datasets --diffusion-steps 1000 --ema_w 0.9995 --ckpt-sample-freq 200 \
    --class-cond --class-cond-dropout 0.1 \
    --save-dir ./logs/ --date 2023-07-31 \
