...
#SBATCH --output=/n/fs/xl-diffbia/projects/minimal-diffusion/slurm_output/2023-01-22/cifar10_train_color0.0_gray1.0.txt            # where stdout and stderr will write to
...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 --master_port $3 main.py \
    --arch UNet --dataset cifar10 --epochs 5000 --batch-size 256 --lr 1e-4 --sampling-steps 1000 \
    --data-dir ./datasets --diffusion-steps 1000 --ema_w 0.9995 --ckpt-sample-freq 500 \
    --save-dir ./logs/ --date 2023-01-22 \
    --color $1 --grayscale $2